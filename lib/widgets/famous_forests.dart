...
class FamousForests extends StatefulWidget {
  @override
  _FamousForestsState createState() => _FamousForestsState();
}

class _FamousForestsState extends State<FamousForests> {
  List<Map<String, dynamic>> FamousForests = [];
  User? user;
  ...
  Future<void> fetchFamousForests() async {
    try {
      final DatabaseReference dbRef = FirebaseDatabase.instance.ref().child('records');
      final snapshot = await dbRef.orderByChild('category').equalTo('Forest').once();

      if (snapshot.snapshot.value != null) {
        final values = snapshot.snapshot.value as Map<dynamic, dynamic>;
        setState(() {
          FamousForests = values.entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount: FamousForests.length,
        itemBuilder: (context, index) {
          final place = FamousForests[index];
