...
class FamousBeaches extends StatefulWidget {
  @override
  _FamousBeachesState createState() => _FamousBeachesState();
}

class _FamousBeachesState extends State<FamousBeaches> {
  List<Map<String, dynamic>> FamousBeaches = [];
  User? user;
  ...
  Future<void> fetchFamousBeaches() async {
    try {
      final DatabaseReference dbRef = FirebaseDatabase.instance.ref().child('records');
      final snapshot = await dbRef.orderByChild('category').equalTo('Beaches').once();

      if (snapshot.snapshot.value != null) {
        final values = snapshot.snapshot.value as Map<dynamic, dynamic>;
        setState(() {
          FamousBeaches = values.entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount: FamousBeaches.length,
        itemBuilder: (context, index) {
          final place = FamousBeaches[index];
