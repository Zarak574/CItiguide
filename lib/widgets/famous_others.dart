...
class FamousOthers extends StatefulWidget {
  @override
  _FamousOthersState createState() => _FamousOthersState();
}

class _FamousOthersState extends State<FamousOthers> {
  List<Map<String, dynamic>> FamousOthers = [];
  User? user;
  ...
  Future<void> fetchFamousOthers() async {
    try {
      final DatabaseReference dbRef = FirebaseDatabase.instance.ref().child('records');
      final snapshot = await dbRef.orderByChild('category').equalTo('Heritage').once();

      if (snapshot.snapshot.value != null) {
        final values = snapshot.snapshot.value as Map<dynamic, dynamic>;
        setState(() {
          FamousOthers = values.entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount: FamousOthers.length,
        itemBuilder: (context, index) {
          final place = FamousOthers[index];
