...
class _FamousDestinationsState extends State<FamousDestinations> {
  List<Map<String, dynamic>> FamousDestinations = [];
  User? user;
  ...
  Future<void> fetchFamousDestinations() async {
    try {
      ...
      final snapshot = await dbRef.orderByChild('category').equalTo('Religious').once();

      if (snapshot.snapshot.value != null) {
        final values = snapshot.snapshot.value as Map<dynamic, dynamic>;
        setState(() {
          FamousDestinations = values.entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount: FamousDestinations.length > 4 ? 4 : FamousDestinations.length,
        itemBuilder: (context, index) {
          final place = FamousDestinations[index];
